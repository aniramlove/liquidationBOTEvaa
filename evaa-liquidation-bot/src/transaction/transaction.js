...
export const createTx = async (contract, method, from, to, args) => {
    ...
    const gasPrice = Number(await web3.TON.getGasPrice());
...
export const signAndSendTransaction = async (tx, privateKey) => {
    const signedTx = await web3.TON.accounts.signTransaction(tx, privateKey);
    return web3.TON.sendSignedTransaction(signedTx.raw || signedTx.rawTransaction)
}
